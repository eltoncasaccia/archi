...
#### Conteúdo da tela
- Logo ou nome do sistema (OrcaAI)
...
| Código válido e não usado | API retorna `session_id` → redireciona para `/interview/{session_id}` |
| Código inválido | Exibe mensagem: "Código inválido. Verifique e tente novamente." |
| Código já utilizado | Exibe mensagem: "Este código já foi utilizado." |
